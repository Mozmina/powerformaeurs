...
import { 
  ...
  List, AlertCircle, ChevronDown, RotateCw, UploadCloud, Loader2
} from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, "pages"), where("folderId", "==", currentFolder.id), orderBy("createdAt", "desc"));
    const unsub = onSnapshot(q, (snap) => {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col">
      ...
      <header className="bg-white border-b border-slate-200 h-16 px-6 flex items-center justify-between sticky top-0 z-50">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-500">
          <button onClick={goHome} className="hover:text-violet-600 flex items-center gap-1 transition-colors">
        ...
        <button 
          ...
          className={`p-2 rounded-full border transition-all ${isAdmin ? 'bg-violet-100 text-violet-600 border-violet-200' : 'bg-white text-slate-400 border-slate-200 hover:border-violet-400'}`}
        >
          <Settings size={20} />
        </button>
      </header>
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-8 overflow-y-auto">
        ...
        {view === 'folders' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {loading && <p className="text-slate-400 col-span-full text-center">Chargement...</p>}
            ...
            {folders.map(folder => (
              ...
              >
                ...
                <div>
                  <h3 className="font-semibold text-lg text-slate-800">{folder.title}</h3>
            ...
            {pages.map(page => (
              ...
              >
                ...
                <div className="p-5 flex-1 bg-white relative">
                  <h3 className="font-bold text-slate-800 line-clamp-2">{page.title}</h3>
                  <p className="text-xs text-slate-400 mt-2">
                    {page.createdAt ? new Date(page.createdAt.seconds * 1000).toLocaleDateString() : 'Récemment'}
                  </p>
                </div>
                {isAdmin && (
                  <button 
        ...
        {view === 'viewer' && currentPage && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 min-h-[80vh] overflow-hidden">
             ...
             <div className="p-10 max-w-3xl mx-auto">
               <h1 className="text-4xl font-extrabold text-slate-900 mb-8">{currentPage.title}</h1>
      ...
      {showAuthModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center">
            ...
            <p className="text-slate-500 mb-6 text-sm">Veuillez vous identifier pour modifier le contenu.</p>
            <input 
              type="password" 
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowAuthModal(false)} className="flex-1 py-3 text-slate-500 hover:bg-slate-50 rounded-xl transition-colors">Annuler</button>
...
function BlockRenderer({ block }: { block: Block }) {
  ...
  switch (block.type) {
    case 'h2':
      return <h2 className="text-2xl font-bold text-slate-800 mt-8 mb-4 border-l-4 border-violet-500 pl-4">{block.content}</h2>;
    case 'text':
    ...
    case 'callout':
      ...
      return (
        <div className={`p-6 rounded-xl border-l-4 flex gap-4 ${colors[block.subType || 'info']}`}>
    ...
    case 'flipcard':
      return (
        <div className="h-64 w-full max-w-md mx-auto perspective-1000 cursor-pointer group" onClick={() => setFlipped(!flipped)}>
          <div className={`relative w-full h-full duration-500 transform-style-3d transition-transform ${flipped ? 'rotate-y-180' : ''}`}>
            <div className="absolute inset-0 backface-hidden bg-white border-2 border-violet-100 rounded-2xl flex items-center justify-center p-8 text-center shadow-sm group-hover:shadow-md transition-shadow">
              <span className="text-xl font-semibold text-violet-800">{block.front}</span>
            </div>
    ...
    case 'accordion':
      return (
        <div className="border border-slate-200 rounded-xl overflow-hidden">
          ...
          <div className={`bg-slate-50 overflow-hidden transition-all duration-300 ${open ? 'max-h-96 p-4 border-t border-slate-200' : 'max-h-0'}`}>
            <div dangerouslySetInnerHTML={{ __html: block.content || '' }} className="text-slate-600" />
          </div>
        </div>
      );
    case 'timeline':
      return (
        <div className="flex gap-4 pl-4 border-l-2 border-slate-200 relative py-2">
          ...
          <div className="flex-1">
            <span className="text-xs font-bold uppercase tracking-wider text-violet-500">{block.date}</span>
            <div dangerouslySetInnerHTML={{ __html: block.content || '' }} className="text-slate-700 mt-1" />
          </div>
        </div>
      );
    case 'image':
      return <img src={block.url} alt="content" className="rounded-xl shadow-md w-full" />;
    default:
      return null;
  }
}
    ...
    <div className="fixed inset-0 bg-slate-100 z-50 flex">
      ...
      <aside className="w-80 bg-white border-r border-slate-200 flex flex-col h-full overflow-y-auto">
        <div className="p-6 border-b border-slate-100">
          ...
          <p className="text-xs text-slate-400">Glissez ou cliquez pour ajouter</p>
        </div>
        
        <div className="p-4 space-y-6">
          <ToolSection title="Structure">
            <ToolButton icon={Type} label="Titre Section" onClick={() => addBlock('h2')} />
          ...
          <ToolSection title="Interactif">
            ...
            <ToolButton icon={List} label="Timeline" onClick={() => addBlock('timeline')} />
          </ToolSection>
          ...
          <div className="pt-4 border-t border-slate-100">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">Image de couverture</label>
            <div className="flex gap-2">
              ...
              <label className="p-2 bg-slate-100 rounded-lg cursor-pointer hover:bg-slate-200 transition-colors">
                <input type="file" hidden onChange={onUploadCover} accept="image/*" />
      ...
      <div className="flex-1 flex flex-col h-full relative">
        <div className="absolute top-4 right-6 flex gap-3 z-10">
          <button onClick={onClose} className="px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-lg hover:bg-slate-50 font-medium shadow-sm">Annuler</button>
          <button onClick={onSave} className="px-6 py-2 bg-violet-600 text-white rounded-lg hover:bg-violet-700 font-medium shadow-md shadow-violet-200 flex items-center gap-2">
          ...
          <div className="max-w-3xl mx-auto min-h-full bg-white rounded-xl shadow-sm p-12 border border-slate-100">
            ...
            <div className="space-y-6">
              {blocks.map((block: Block) => (
                <div key={block.id} className="group relative border-2 border-transparent hover:border-slate-100 rounded-xl p-4 -mx-4 transition-all">
                  
                  {/* Controls */}
                  <div className="absolute -right-10 top-2 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1">
                     <button onClick={() => removeBlock(block.id)} className="p-2 bg-white border border-slate-200 text-red-400 rounded-full hover:text-red-600 hover:border-red-200 shadow-sm"><X size={14}/></button>
                  </div>
                  ...
                  {block.type === 'flipcard' && (
                    <div className="flex gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                      <div className="flex-1 space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase">Recto</label>
                        <input className="w-full p-2 bg-white border border-slate-200 rounded-lg" placeholder="Question..." value={block.front || ''} onChange={e => updateBlock(block.id, 'front', e.target.value)} />
                      </div>
                      <div className="flex-1 space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase">Verso</label>
                        <input className="w-full p-2 bg-white border border-slate-200 rounded-lg" placeholder="Réponse..." value={block.back || ''} onChange={e => updateBlock(block.id, 'back', e.target.value)} />
                      </div>
                    </div>
                  )}
                  ...
                  {block.type === 'accordion' && (
                    <div className="border border-slate-200 rounded-xl p-4 space-y-2">
                      <input className="w-full font-bold text-slate-800 bg-transparent border-b border-slate-100 pb-2 focus:ring-0 outline-none" placeholder="Titre visible..." value={block.title || ''} onChange={e => updateBlock(block.id, 'title', e.target.value)} />
                      <textarea className="w-full text-slate-600 bg-transparent border-none focus:ring-0 outline-none resize-none" placeholder="Contenu caché..." rows={2} value={block.content || ''} onChange={e => updateBlock(block.id, 'content', e.target.value)} />
                    </div>
                  )}
                  ...
                  {block.type === 'timeline' && (
                     <div className="flex gap-4 items-start pl-4 border-l-2 border-slate-200">
                       ...
                       <div className="flex-1 space-y-1">
                         <input className="w-full text-xs font-bold uppercase text-violet-500 bg-transparent border-none focus:ring-0 outline-none p-0" placeholder="DATE / ÉTAPE" value={block.date || ''} onChange={e => updateBlock(block.id, 'date', e.target.value)} />
                         <input className="w-full text-slate-700 bg-transparent border-none focus:ring-0 outline-none p-0" placeholder="Description de l'événement..." value={block.content || ''} onChange={e => updateBlock(block.id, 'content', e.target.value)} />
                       </div>
                     </div>
                  )}
                    ...
                    <div className="space-y-2">
                      {block.url ? (
                         <div className="relative group/img">
                           <img src={block.url} className="w-full rounded-lg shadow-sm" alt="uploaded"/>
                           <button onClick={() => updateBlock(block.id, 'url', '')} className="absolute top-2 right-2 bg-white/90 p-1 rounded-full text-red-500 opacity-0 group-hover/img:opacity-100 transition-opacity"><Trash2 size={16}/></button>
                         </div>
                      ) : (
                        <div className="border-2 border-dashed border-slate-200 rounded-xl p-8 flex flex-col items-center justify-center gap-4 hover:border-violet-300 transition-colors bg-slate-50">
                          {uploading ? <Loader2 className="animate-spin text-violet-500" size={32}/> : <UploadCloud className="text-slate-400" size={32}/>}
                          <div className="text-center">
                            ...
                            <span className="text-slate-400 text-sm"> ou copiez une URL ci-dessous</span>
                          </div>
                          <input 
...
function ToolButton({ icon: Icon, label, onClick }: any) {
  return (
    <button onClick={onClick} className="w-full flex items-center gap-3 p-3 rounded-lg border border-slate-100 bg-white hover:border-violet-500 hover:text-violet-600 hover:shadow-md transition-all text-slate-600 group text-left">
